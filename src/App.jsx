...
const BeforeAfterSlider = () => {
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto overflow-hidden rounded-[2.5rem] shadow-2xl aspect-[16/9] group cursor-col-resize select-none touch-none"
      ...
      <div className="absolute inset-0 w-full h-full">
        ...
        <div className="absolute bottom-8 right-8 bg-emerald-600 text-white px-5 py-2 rounded-full text-xs font-black shadow-lg uppercase tracking-tighter">After Reconstruction</div>
      </div>
      <div className="absolute inset-0 w-full h-full overflow-hidden transition-none border-r-4 border-white" style={{ width: `${sliderPos}%` }}>
        ...
        <div className="absolute bottom-8 left-8 bg-slate-800 text-white px-5 py-2 rounded-full text-xs font-black shadow-lg uppercase tracking-tighter">Before</div>
      </div>
      <div className="absolute inset-y-0 z-10 w-1 bg-white" style={{ left: `${sliderPos}%` }}>
        <div className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 bg-white text-slate-900 p-3 rounded-full shadow-2xl ring-8 ring-white/20"><Maximize2 className="w-6 h-6 rotate-45" /></div>
      </div>
    </div>
  );
};
        ...
        <div className="p-10">
              ...
              <div className="grid gap-3">
                {['Driveway', 'Patio/Walkway', 'Repair'].map(type => (
                  <button key={type} onClick={() => { setFormData({...formData, type}); setStep(2); }}
                    className="flex items-center justify-between p-5 border-2 border-slate-50 rounded-2xl hover:border-emerald-500 transition-all font-bold text-slate-700">
              ...
              <div className="grid grid-cols-2 gap-3">
                {['<500 sqft', '500-1k', '1k-2.5k', '2.5k+'].map(size => (
                  <button key={size} onClick={() => { setFormData({...formData, size}); setStep(3); }}
                    className="p-5 border-2 border-slate-50 rounded-2xl text-center hover:border-emerald-500 transition-all font-bold text-slate-700">{size}</button>
                ))}
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-3">
                <input required placeholder="Your Name" className="w-full p-5 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 ring-emerald-500" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                <input required placeholder="Phone Number" type="tel" className="w-full p-5 bg-slate-50 border-none rounded-2xl outline-none focus:ring-2 ring-emerald-500" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} />
                <button type="submit" disabled={isSubmitting} className="w-full bg-emerald-600 text-white py-5 rounded-2xl font-black text-lg hover:bg-emerald-700 shadow-xl flex items-center justify-center gap-2">
          ...
          {step === 4 && (
            <div className="text-center py-8 space-y-4">
              ...
              <p className="text-slate-500">Todd will reach out within 24 hours.</p>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 font-sans selection:bg-emerald-100 selection:text-emerald-900">
      ...
      {/* Navigation */}
      <nav className={`fixed w-full z-40 transition-all duration-500 ${isScrolled ? 'bg-white/95 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-8'}`}>
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          >
            <div className="bg-slate-900 p-2.5 rounded-xl group-hover:bg-emerald-600 transition-colors">
              <Construction className="text-white w-6 h-6" />
            </div>
            <div>
              <span className="block font-black text-xl uppercase tracking-tighter text-slate-900">Exterior Designs</span>
              <span className="block text-[10px] font-bold tracking-[0.25em] text-emerald-600 uppercase">Pavers & Hardscape</span>
            </div>
          </div>

          {/* Expanded Desktop Nav */}
          <div className="hidden md:flex items-center gap-8 text-xs font-black uppercase tracking-[0.2em] text-slate-600">
            ...
            <button onClick={() => setIsQuoteOpen(true)} className="bg-emerald-600 text-white px-8 py-4 rounded-full hover:bg-slate-900 transition-all shadow-xl shadow-emerald-50 transform hover:-translate-y-0.5">Get a Quote</button>
          </div>
        ...
        {mobileMenu && (
          <div className="absolute top-full left-0 w-full bg-white border-t border-slate-100 p-6 flex flex-col gap-6 md:hidden shadow-2xl animate-in slide-in-from-top duration-300">
             <a href="#services" onClick={(e) => smoothScroll(e, 'services')} className="font-bold text-slate-600 text-lg">Services</a>
             <a href="#work" onClick={(e) => smoothScroll(e, 'work')} className="font-bold text-slate-600 text-lg">Portfolio</a>
             <a href="#about" onClick={(e) => smoothScroll(e, 'about')} className="font-bold text-slate-600 text-lg">About Us</a>
             <a href="#coverage" onClick={(e) => smoothScroll(e, 'coverage')} className="font-bold text-slate-600 text-lg">Coverage</a>
             <a href="#faq" onClick={(e) => smoothScroll(e, 'faq')} className="font-bold text-slate-600 text-lg">FAQ</a>
             <div className="flex gap-8 py-4 border-y border-slate-50">
                <a href={BUSINESS_DATA.social.facebook} target="_blank" rel="noopener noreferrer" className="text-slate-400"><FacebookIcon className="w-8 h-8" /></a>
                <a href={BUSINESS_DATA.social.yelp} target="_blank" rel="noopener noreferrer" className="text-rose-600"><YelpIcon className="w-8 h-8" /></a>
             </div>
             <button onClick={() => { setMobileMenu(false); setIsQuoteOpen(true); }} className="bg-emerald-600 text-white py-5 rounded-2xl font-black text-lg">Get a Quote</button>
          </div>
        )}
      </nav>

      {/* Hero Section */}
      <header className="relative pt-32 pb-20 md:pt-56 md:pb-32 overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <div className="relative z-10 space-y-10">
            <div className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border border-emerald-100">
              <MapPin className="w-3.5 h-3.5" /> San Carlos & Peninsula
            </div>
            <h1 className="text-6xl md:text-8xl font-black text-slate-900 leading-[0.85] tracking-tighter">
              Timeless <br /><span className="text-emerald-600">Hardscapes.</span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-5">
              <button onClick={() => setIsQuoteOpen(true)} className="group flex items-center justify-center gap-3 bg-slate-900 text-white px-10 py-6 rounded-[2rem] font-black text-lg hover:bg-emerald-600 transition-all shadow-2xl shadow-slate-200">
                Instant Estimate <ArrowRight className="w-5 h-5 group-hover:translate-x-1" />
              </button>
              <a href={`tel:${BUSINESS_DATA.phone.replace(/\D/g,'')}`} className="flex items-center justify-center gap-3 bg-white border-2 border-slate-100 text-slate-900 px-10 py-6 rounded-[2rem] font-black text-lg hover:border-emerald-600 transition-all">
          ...
          <div className="relative">
             ...
             <img src="/images/exterior-designs-hero.png" alt="Luxury Paver Installation" className="rounded-[3.5rem] shadow-2xl object-cover aspect-[4/5] lg:aspect-[3/4] border-8 border-white" />
          </div>
        </div>
      </header>
      ...
      <section className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6 text-center mb-16 space-y-4">
          <h2 className="text-4xl md:text-5xl font-black tracking-tight text-slate-900">The Transformation</h2>
          <p className="text-slate-500 text-lg max-w-2xl mx-auto mb-12">Slide to see the difference between cracked concrete and our premium pavers.</p>
          <BeforeAfterSlider />
        </div>
      </section>

      {/* ICPI Standards Section */}
      <section id="icpi-standards" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-16 items-center">
            <div className="lg:w-1/2 space-y-8">
              <div className="flex items-center gap-4">
                ...
                <span className="text-emerald-600 font-black uppercase tracking-[0.2em] text-[10px]">Technical Excellence</span>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-8">
                {ICPI_STEPS.map((step, idx) => (
                  <div key={idx} className="space-y-3">
                    <div className="w-10 h-10 bg-slate-900 text-emerald-500 rounded-lg flex items-center justify-center shadow-lg">
                      {step.icon}
                    </div>
            ...
            <div className="lg:w-1/2 bg-slate-950 p-12 rounded-[3rem] text-white relative overflow-hidden">
               <Award className="w-12 h-12 text-emerald-500 mb-8" />
               <h3 className="text-3xl font-black mb-4 tracking-tight">Engineering for Longevity</h3>
               <p className="text-slate-400 text-sm leading-relaxed mb-8">By adhering to engineering guidelines, we guarantee a flexible pavement system that withstands heavy loads better than traditional concrete.</p>
               <div className="pt-8 border-t border-white/10 flex justify-between">
                  <div><div className="text-xl font-black">98%</div><div className="text-[10px] text-slate-500 uppercase mt-1">Compaction</div></div>
                  <div><div className="text-xl font-black">30+ Yrs</div><div className="text-[10px] text-slate-500 uppercase mt-1">Lifespan</div></div>
                  <div><div className="text-xl font-black">ICPI</div><div className="text-[10px] text-slate-500 uppercase mt-1">Certified</div></div>
               </div>
            </div>
        </div>
      </section>
      ...
      <section id="services" className="py-24 bg-slate-50">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16 space-y-4">
             ...
             <h2 className="text-4xl md:text-5xl font-black tracking-tight text-slate-900">What We Build.</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
             {SERVICES_LIST.map((service, idx) => (
               <div key={idx} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center gap-4 hover:border-emerald-500 transition-colors group">
                  <div className="w-12 h-12 bg-slate-50 text-slate-400 group-hover:bg-emerald-50 group-hover:text-emerald-600 rounded-2xl flex items-center justify-center">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-8">
              ...
              <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-slate-900 leading-none">A Direct Partnership <br/><span className="text-emerald-600">With the Owners.</span></h2>
              <div className="space-y-6 text-slate-600 text-lg font-medium leading-relaxed">
                <p>
                  We are a family owned and operated construction company providing our clients with the best customer service and craftsmanship in the industry. By working with Exterior Designs you have the unique experience of working directly with the owners of the company, ensuring you receive the best experience possible for your hardscape project.
                </p>
                <div className="p-8 bg-emerald-50 rounded-[2.5rem] border border-emerald-100 space-y-6">
                  <h3 className="text-2xl font-black text-emerald-900">Feel Safe</h3>
                  <p className="text-emerald-800/80 leading-relaxed italic">
                    "When you work with Exterior Designs, you get to work directly with the owners of the company. You have the comfort of having them just a phone call away, so you don't have to chase down the chain of command to get the answers you need about your project."
                  </p>
                  <div className="pt-4 border-t border-emerald-200 flex items-center gap-4">
                    <span className="font-black text-emerald-900 uppercase tracking-widest text-sm">— Todd and Flavio</span>
                  </div>
                </div>
              </div>
            </div>
            <div className="relative">
              <img src="/images/exterior-designs-hero.png" alt="Direct Partnership" className="rounded-[3rem] shadow-2xl border-8 border-white" />
            </div>
          </div>
        </div>
      </section>

      {/* Support Your Green Company - FIXED CLIPPING: Safe container cell */}
      <section id="environmental" className="py-24 bg-slate-900 text-white relative">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-20 items-center">
            <div className="order-2 lg:order-1 flex justify-center items-center">
              <div className="relative w-full max-w-2xl p-4 lg:p-8">
                ...
                <img 
                  ...
                  className="relative w-full h-auto shadow-2xl border-2 border-white/5 z-10 block" 
                />
              </div>
            </div>
            <div className="space-y-8 order-1 lg:order-2">
              ...
              <div className="space-y-6 text-slate-400 text-lg leading-relaxed font-medium">
                ...
                <div className="p-6 bg-white/5 rounded-3xl border border-white/10 space-y-4">
                  <p className="text-sm italic">"We donate leftover material to Habitat for Humanity. We sweep gutters, filter drains, and wet things down to minimize hazardous dust."</p>
                  <p className="text-sm font-bold text-white uppercase tracking-widest pt-4 border-t border-white/10">When it comes to the environment, we simply do not take shortcuts.</p>
                </div>
              </div>
            </div>
        </div>
      ...
      <section id="team" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16 space-y-4">
            <h2 className="text-4xl md:text-5xl font-black tracking-tight text-slate-900">Meet Todd & Flavio.</h2>
          </div>
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
              {TEAM.map((member, idx) => (
                <div key={idx} className="group relative bg-slate-50 rounded-[3rem] overflow-hidden border border-slate-100 hover:shadow-2xl transition-all">
                  ...
                  <div className="p-10 space-y-4 relative bg-white">
                    <div className="inline-block bg-emerald-600 text-white px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">{member.role}</div>
      ...
      <section id="faq" className="py-24 bg-slate-50">
        <div className="max-w-3xl mx-auto px-6">
          <div className="text-center mb-16 space-y-4">
            ...
            <h2 className="text-4xl md:text-5xl font-black tracking-tight text-slate-900">Frequently Asked.</h2>
          </div>
          <div className="space-y-6">
            {FAQ_DATA.map((item, idx) => (
              <div key={idx} className="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm hover:border-emerald-500 transition-colors duration-300">
                ...
                <p className="text-slate-500 leading-relaxed pl-10">{item.a}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="coverage" className="py-24 bg-white overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
          <div className="space-y-8">
            ...
            <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-slate-900 leading-none">Serving Your <br/><span className="text-emerald-600">Neighborhood.</span></h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-8">
               {SERVICE_AREAS.map((city, idx) => (
                 <div key={idx} className="flex items-center gap-2 text-slate-700 font-bold group">
          ...
          <div className="bg-slate-900 p-12 rounded-[4rem] text-white space-y-12">
             <div className="space-y-4">
                <Shield className="w-12 h-12 text-emerald-500" />
                <h3 className="text-3xl font-black tracking-tight">The Lifetime Promise</h3>
                <p className="text-slate-400">Our Interlocking Hardscapes are engineered to last a lifetime. We follow a technical compaction process that prevents shifting, even under the heaviest loads.</p>
             </div>
             <div className="grid grid-cols-2 gap-8 pt-8 border-t border-white/10">
                <div>
                  <div className="text-2xl font-black text-white">98%</div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mt-1">Compaction Density</div>
                </div>
                <div>
                  <div className="text-2xl font-black text-white">Zero</div>
                  <div className="text-[10px] font-black uppercase tracking-widest text-emerald-500 mt-1">Shortcut Policy</div>
                </div>
             </div>
          </div>
        </div>
      ...
      <section className="py-24 bg-slate-50 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row items-center justify-between mb-16 gap-8 text-center md:text-left">
            <div className="space-y-4">
              <h2 className="text-4xl md:text-5xl font-black tracking-tight text-slate-900 leading-none">Trusted Across the <span className="text-emerald-600">Peninsula.</span></h2>
              <p className="text-slate-500 text-lg font-medium">Verified 5-star feedback from your local neighbors.</p>
            </div>
            <a href={BUSINESS_DATA.social.yelp} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 bg-white px-8 py-4 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all font-bold group text-slate-700">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {TESTIMONIALS.map((t, idx) => (
              <div key={idx} className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100 flex flex-col justify-between group hover:-translate-y-2 transition-transform duration-500">
                <div className="space-y-6">
                  ...
                  <div className="relative">
                    <QuoteIcon className="absolute -top-4 -left-4 w-12 h-12 text-slate-50 -z-0" />
                    <p className="relative z-10 text-slate-700 leading-relaxed font-medium italic">"{t.text}"</p>
                  </div>
                </div>
                <div className="mt-10 pt-6 border-t border-slate-50 flex items-center gap-4">
                   <div className="w-10 h-10 bg-emerald-600 text-white rounded-full flex items-center justify-center font-black text-lg">
                    {t.author.charAt(0)}
                   </div>
                   <div>
                    <h5 className="font-bold text-slate-900 leading-none">{t.author}</h5>
                    <span className="text-[10px] uppercase font-black tracking-widest text-emerald-600 mt-1 block">{t.location}</span>
                   </div>
                </div>
              </div>
            ))}
      ...
      <section id="work" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-6 text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-black tracking-tight text-slate-900 mb-4">Our Craftsmanship</h2>
          <div className="flex flex-wrap justify-center gap-2">
            {['All', 'Driveways', 'Patios'].map(cat => (
              <button key={cat} onClick={() => setFilter(cat)} className={`px-6 py-2 rounded-full text-sm font-bold transition-all ${filter === cat ? 'bg-emerald-600 text-white shadow-lg' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>{cat}</button>
            ))}
          </div>
        </div>
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredProjects.map((project) => (
            <div 
              ...
              className="group relative overflow-hidden rounded-[2rem] bg-slate-100 aspect-[4/5] cursor-zoom-in"
            >
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950/80 via-slate-950/40 to-transparent p-8 text-white translate-y-4 group-hover:translate-y-0 transition-transform">
                ...
                <div className="flex items-center gap-2 text-slate-300 text-sm font-medium"><MapPin className="w-4 h-4 text-emerald-500" /> {project.location}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Footer */}
      <footer className="pt-32 pb-16 bg-slate-900 text-white">
        <div className="max-w-7xl mx-auto px-6 space-y-24">
          <div className="flex flex-col lg:flex-row justify-between items-center gap-12 bg-white/5 backdrop-blur-md p-16 rounded-[4rem] text-center lg:text-left">
            <div className="space-y-6">
              <h2 className="text-4xl md:text-6xl font-black tracking-tighter leading-tight">Ready for a <br/>Better Exterior?</h2>
              <p className="text-xl text-slate-400 font-medium">Contact Todd directly for an on-site consultation.</p>
            </div>
            <button onClick={() => setIsQuoteOpen(true)} className="bg-emerald-600 text-white px-12 py-7 rounded-[2rem] font-black text-2xl hover:bg-white hover:text-emerald-600 transition-all shadow-2xl">Start My Quote</button>
          </div>
          <div className="flex flex-col md:flex-row justify-between items-center pt-12 border-t border-white/10 text-slate-500 text-[10px] font-black uppercase tracking-[0.2em] gap-8">
            <div className="flex items-center gap-3">© {new Date().getFullYear()} {BUSINESS_DATA.name}</div>
            <div className="flex gap-8 items-center flex-wrap justify-center font-bold">
              <a href={BUSINESS_DATA.social.facebook} target="_blank" rel="noopener noreferrer" className="hover:text-emerald-400 transition-colors">
                <FacebookIcon className="w-5 h-5" />
              </a>
              <a href={BUSINESS_DATA.social.yelp} target="_blank" rel="noopener noreferrer" className="hover:text-emerald-400 transition-colors">
      ...
      {selectedImg && (
        <div 
          className="fixed inset-0 z-[60] bg-slate-950/95 backdrop-blur-xl flex items-center justify-center p-4 md:p-12 animate-in fade-in duration-300"
